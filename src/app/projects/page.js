          ...
          <div key={project.id} className="grid lg:grid-cols-12 gap-8 items-stretch">
            <div className="lg:col-span-4 flex flex-col">
              ...
              <div className={`mt-4 space-y-3 p-4 border-2 ${
                ...
                {(theme === 'profdr' || theme === 'enterprise' || theme === 'vax') && <div className={`text-[10px] font-bold border-b mb-2 pb-1 ${
                ...
                {Object.entries(project.stats).slice(0, 3).map(([key, val]) => (
                  <div key={key} className="space-y-1">
                    <div className={`flex justify-between text-[10px] font-bold opacity-70 uppercase tracking-tighter ${
                      theme === '2010s' || theme === 'profdr' || theme === 'enterprise' || theme === 'vax' ? 'text-inherit opacity-100' : ''
                    }`}>
                      <span>{key}</span>
                      <span>{val}</span>
                    </div>
                      ...
                      <div className={`h-1.5 w-full overflow-hidden border ${
                        ...
                        <div 
                          className={`h-full transition-all duration-1000 ${
                            theme === 'profdr' || theme === 'enterprise' ? 'bg-blue-800' :
              ...
              <Card title={project.name.toUpperCase()} className="h-full">
                <div className="space-y-6">
                  ...
                  <div className={`grid grid-cols-2 sm:grid-cols-3 gap-4 border-b pb-4 ${
                    theme === 'profdr' || theme === 'enterprise' ? 'border-gray-100' :
                    ...
                    {Object.entries(project.stats).map(([key, val]) => (
                      <div key={key}>
                        <span className={`block text-[10px] font-bold uppercase tracking-widest opacity-40 ${
                          theme === '2010s' || theme === 'profdr' || theme === 'enterprise' || theme === 'vax' ? 'text-inherit opacity-60' : ''
                        }`}>[{key}]</span>
                        <span className={`text-xl font-bold ${
                          theme === '2010s' || theme === 'profdr' || theme === 'enterprise' || theme === 'vax' ? 'text-inherit' : ''
                        }`}>{val}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <p className={`text-2xl leading-snug font-medium ${
                    theme === 'profdr' || theme === 'enterprise' ? 'font-serif text-black' :
                  ...
                  <div className="space-y-4">
                    <h4 className={`font-bold text-xs border-l-4 pl-2 ${
                      theme === 'profdr' || theme === 'enterprise' ? 'text-blue-800 border-blue-800' :
                      ...
                      {project.powers.map(power => (
                        <div key={power.name} className={`p-3 border ${
                          ...
                          <span className={`text-sm opacity-80 leading-tight block ${
                            theme === '2010s' || theme === 'profdr' || theme === 'enterprise' || theme === 'vax' ? 'text-inherit opacity-70' : ''
                          }`}>
                            {power.desc}
                          </span>
                        </div>
                  ...
                  <div className="flex flex-wrap gap-4 pt-4">
                    {Object.entries(project.links).map(([type, url]) => (
                      <a key={type} href={url} target="_blank" rel="noopener noreferrer" className={
                        ...
                        {theme === 'profdr' || theme === 'enterprise' || theme === 'vax' ? type.toUpperCase() : type === 'github' ? 'ACCESS_SOURCE' : `DEPLOY_${type.toUpperCase()}`}
                      </a>
                    ))}
                  </div>
                </div>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div>
              ...
              <div className={`space-y-4 font-medium text-xl opacity-90 ${
                theme === 'enterprise' ? 'text-sm text-[#212529]' : 
                ...
                <p>
                  We are close collaborators with the <a href="https://usegalaxy.org" target="_blank" rel="noopener noreferrer" className={`${
                    theme === 'vax' ? 'underline' : 'text-blue-500 hover:underline decoration-2 underline-offset-4'
              ...
              <div className={`space-y-4 font-medium text-xl opacity-90 ${
                ...
                <div className="pt-2">
                  <a href="https://github.com/veg" target="_blank" rel="noopener noreferrer" className={
                    theme === 'enterprise' ? 'btn-secondary enterprise-button text-xs' :
                    theme === 'vax' ? 'vax-button text-xs' :
                    ...
                    {theme === 'enterprise' || theme === 'vax' ? (theme === 'vax' ? 'DIR REPOS.LOG' : 'Browse Catalog on GitHub') : 'BROWSE_FULL_CATALOG_ON_GITHUB'}
                  </a>
                </div>
              </div>
            </div>
