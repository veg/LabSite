            ...
            <div className={`mb-4 relative h-48 w-full overflow-hidden ${
              ...
              {member.image && theme !== 'vax' ? (
                <Image 
                  ...
                  alt={member.name}
                  fill
                  className={`object-cover ${theme === 'profdr' || theme === 'enterprise' || theme === 'knuth' ? '' : 'filter brightness-90 contrast-110'}`}
                  unoptimized
                />
              ) : (
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(member.links).map(([type, url]) => (
                <a key={type} href={url} target="_blank" rel="noopener noreferrer" className={
                  ...
                  {theme === 'profdr' || theme === 'enterprise' || theme === 'vax' || theme === 'knuth' ? type.toUpperCase() : `[${type}]`}
                </a>
              ))}
            </div>
          </Card>
